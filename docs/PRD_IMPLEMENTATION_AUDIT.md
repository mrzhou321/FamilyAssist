...
| Photo quick notes | `QuickNote.tsx` uses `input accept="image/*" capture="environment"`, records photo metadata in note text, and persists a compressed `photo_thumbnail` with the note for review/source evidence. Mobile viewport smoke checks the built mobile shell route and pairing route. | Covered for capture metadata and thumbnail retention; no OCR or original binary archive by PRD scope |
