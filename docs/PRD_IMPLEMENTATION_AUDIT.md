...
| Photo quick notes | `QuickNote.tsx` uses `input accept="image/*" capture="environment"` and records photo metadata in note text. Mobile viewport smoke checks the built mobile shell route and pairing route. | Covered for capture metadata; no OCR or binary archive by PRD scope |
