...
- Automated mobile viewport smoke: `powershell -ExecutionPolicy Bypass -File scripts\mobile-viewport-smoke.ps1`
...
| Voice quick notes | `QuickNote.tsx` uses `SpeechRecognition` / `webkitSpeechRecognition` and appends transcript as `source: "voice"`. Mobile viewport smoke verifies the PWA shell routes in a real browser size. | Covered by code wiring; device/browser compatibility still needs manual mobile testing |
| Photo quick notes | `QuickNote.tsx` uses `input accept="image/*" capture="environment"`, records photo metadata in note text, persists a compressed `photo_thumbnail`, and shows thumbnails in the recent-note list plus review/source evidence. Mobile viewport smoke checks the built mobile shell route and pairing route. | Covered for capture metadata and thumbnail retention; no OCR or original binary archive by PRD scope |
...
| Memory metadata | `Memory` has `source_note_id`, `confidence`, `expires_at`, `domain`, embedding. Admin memory library shows source and expiry and can edit expiry; mobile memory vault can open the original source note and photo thumbnail evidence. | Covered |
...
| Member data isolation | Backend scopes member routes by JWT payload; paired members can read their own data plus `member_id=None` family-shared memories/source notes, but not other members' data. Member deletion clears sessions and pairing tokens while preserving historical notes/memories as family history; self-check covers forbidden cross-member access, family-shared memory visibility, revoked sessions, and deletion cleanup. | Covered |
...
- Voice recognition, camera capture, and QR permission prompts still depend on real mobile browser behavior and should be manually tested on target Android/iOS browsers using `docs\MANUAL_ACCEPTANCE.md`; `scripts\mobile-viewport-smoke.ps1` covers only mobile-sized browser rendering.
...
- Photo support captures metadata, user description, and a compressed review thumbnail, not image OCR or original binary storage.
